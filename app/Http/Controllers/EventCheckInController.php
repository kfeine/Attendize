...
class EventCheckInController extends MyBaseController
{
    ...
    public function postCheckInAttendee(Request $request)
    {
        ...
        if ((($checking == 'in') && ($attendee->has_arrived == 1)) || (($checking == 'out') && ($attendee->has_arrived == 0))) {
            return response()->json([
                'status'  => 'error',
                'message' => __('controllers_eventcheckincontroller.already_checked') . (($checking == 'in') ? 'In (at ' . $attendee->arrival_time->format('H:i A, F j') . ')' : 'Out') . '!',
                'checked' => $checking,
                'id'      => $attendee->id,
            ]);
        }
        ...
        return response()->json([
            'status'  => 'success',
            'checked' => $checking,
            'message' => __('controllers_eventcheckincontroller.already_checked') . (($checking == 'in') ? 'In' : 'Out'),
            'id'      => $attendee->id,
        ]);
    }
    ...
    public function postCheckInAttendeeQr($event_id, Request $request)
    {
        ...
        if (is_null($attendee)) {
            return response()->json([
                'status'  => 'error',
                'message' => __('controllers_eventcheckincontroller.invalid_ticket')
            ]);
        }
        ...
        if ($attendee->has_arrived) {
            return response()->json([
                'status'  => 'error',
                'message' => __('controllers_eventcheckincontroller.already_checked') . $attendee->arrival_time->format('H:i A, F j') . $appendedText
            ]);
        }
