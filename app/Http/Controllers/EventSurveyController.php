...
class EventSurveyController extends MyBaseController
{
    ...
    public function postCreateEventQuestion(StoreEventQuestionRequest $request, $event_id)
    {
        ...
        session()->flash('message', __('controllers_eventsurveycontroller.create_success'));

        return response()->json([
            'status'      => 'success',
            'message'     => __('controllers_eventsurveycontroller.refreshing'),
            'redirectUrl' => '',
        ]);
    }
    ...
    public function postEditEventQuestion(Request $request, $event_id, $question_id)
    {
        ...
        $question                   = Question::scope()->findOrFail($question_id);
        $question->title            = $request->get('title');
        $question->is_required      = ($request->get('is_required') == 1);
        $question->question_type_id = $request->get('question_type_id');
        $question->save();
        ...
        session()->flash('message', __('controllers_eventsurveycontroller.edit_success'));
    ...
    public function postDeleteEventQuestion(Request $request, $event_id)
    {
        ...
        return response()->json([
            'status'  => 'error',
            'id'      => $question->id,
            'message' => __('controllers_eventsurveycontroller.delete_errror'),
        ]);
    }
    ...
    public function showExportAnswers(Request $request, $event_id, $export_as = 'xlsx')
    {
        Excel::create('answers-as-of-' . date('d-m-Y-g.i.a'), function ($excel) use ($event_id) {
            ...
            $excel->setTitle(__('controllers_eventsurveycontroller.survey_answers'));
    ...
    public function postEnableQuestion(Request $request, $event_id, $question_id)
    {
        ...
        if ($question->save()) {
            return response()->json([
                'status'  => 'success',
                'message' => __('controllers_eventsurveycontroller.update_success'),
                'id'      => $question->id,
            ]);
        }
        ...
        return response()->json([
            'status'  => 'error',
            'id'      => $question->id,
            'message' => __('controllers_eventsurveycontroller.error'),
        ]);
    }
