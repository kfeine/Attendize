...
\newcommand*{\SingleBadge}[1]{
  ...
  \begin{tikzpicture}
    ...
    % Name and origin
    \node [anchor=south] at (\BadgeWidth / 2, 2.0) {\textbf{\LARGE{\StrDel{\firstname~\lastname}{"}}}};
    % \node [anchor=south] at (\BadgeWidth / 2, 1.3) {\large{\church}};
    \node [anchor=south] at (\BadgeWidth / 2, 0.7) {\large{\StrDel{\postalcode~- \city}{"}}};
  \end{tikzpicture}
  };
  ...
  \begin{tikzpicture}
    ...
    % ticket
    \node[anchor=west] at (0.7cm,\BadgeHeight - 1cm) {\textbf{\StrDel{\ticket}{"}}};

    % room
    \ifthenelse{ \equal{\room}{} }{ }{
      \node[anchor=west, text width=\BadgeWidth*0.7, align=left] at (0.7cm,\BadgeHeight - 2.5cm) {\StrDel{\room}{"}};
    }

    % meals
    \draw (0.8cm, \BadgeHeight - 6cm)
...
\begin{document}
  ...
  \csvreader{attendees.csv}{%
    ...
    19=\mealC, % sam soir
    20=\mealD, % dim mid
    22=\mealAB, % ven soir < 10 ans
    23=\mealBB, % sam mid < 10 ans
    24=\mealCB, % sam soir < 10 ans
    21=\mealDB}% dim midi < 10 ans
  {
