    ...
    <div class="col-md-12">
        <div class="form-group">
            ...
            @elseif($question->question_type_id == config('attendize.question_dropdown_single'))
                {!! Form::select("ticket_holder_questions[{$ticket->id}][{$attendee_id}][$question->id]", array_merge(['' => __('public_viewevent_partials_ticketoptions.select_one_option')], $question->options->sortBy('name')->pluck('name', 'name')->toArray()), null, [$question->is_required ? 'required' : '' => $question->is_required ? 'required' : '', 'class' => "ticket_holder_questions.{$ticket->id}.{$attendee_id}.{$question->id} form-control"]) !!}
